...
public class ReviewService : IReviewService
{
    ...
    public async Task<IEnumerable<GetReviewDTO>> GetReviewsByProductIdAsync(Guid productId)
    {
    ...
    {
        var review = await _unitOfWork.GetRepository<ProductReview, Guid>().GetByIdAsync(id);
        if (review is null) throw new KeyNotFoundException($"Review with ID {id} not found.");
        return _mapper.Map<GetReviewDTO>(review);
    }

    public async Task<GetReviewDTO> AddReviewAsync(string userId, AddReviewDTO dto)
    {
        var result = await _addValidator.ValidateAsync(dto);
        if (!result.IsValid) throw new ValidationException(result.Errors);
        var review = _mapper.Map<ProductReview>(dto);
        review.UserId = userId;
        await _unitOfWork.GetRepository<ProductReview, Guid>().AddAsync(review);
        await _unitOfWork.SaveChangesAsync();
        return _mapper.Map<GetReviewDTO>(review);
    }

    public async Task DeleteReviewAsync(string userId, Guid id)
    {
        ...
        _unitOfWork.GetRepository<ProductReview, Guid>().Delete(stub);
        await _unitOfWork.SaveChangesAsync();
    }
}
